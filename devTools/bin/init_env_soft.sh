...
msun='/data'
msun_static='/data/static'
msun_run='/data/run'
msun_run_static='/data/run/static'
app_user_passwd='msun_app_pwd_1@'
...
function apt_yum_install_tools {
    yum install subversion vim curl tree ntpdate 
}
function go_or_exit {
...
function add_apps_user {
    ...
    do
        mkdir -p ${msun}/${i}
        adduser ${i} -g admin -d ${msun}/${i}
        echo ${app_user_passwd} | passwd ${i} --stdin
        chown -R ${i}.admin ${msun}/${i}
        chmod 755 ${msun}/${i}
    done
...
function cp_msun_run_static {
    cp -r ${msun_run_static} ${msun}
    chown -R admin.admin ${msun_static}
    chown -R nobody.nobody "${msun_static}/logs"
}
function init_d {
    for i in memcached nginx manage_agent
    do
        cp "${msun_run}/bin/${i}" /etc/init.d/
        chkconfig --add ${i}
...
function nfs_server {
    ...
    if [ "$ans" == 'y' ] || [ "$ans" == 'yes' ]; then
        mkdir -p /msun/nfs
        chown nfsnobody.nfsnobody /msun/nfs
        cat >> /etc/exports <<EOF
/msun/nfs 192.168.1.1/255.255.255.0(rw,all_squash,anonuid=65534,anongid=65534)
EOF
...
function nfs_client {
    #cat >> /etc/fstab <<EOF
#192.168.1.101:/msun/nfs /msun/static/nfs    nfs rsize=8192,wsize=8192,timeo=14,intr
#EOF
    #mount -a
    echo 'empty'
}
function rsync_server {
    ...
    if [ "$ans" == 'y' ] || [ "$ans" == 'yes' ]; then
        cp "${msun_run}/bin/rsyncd" /etc/init.d/
        cp "${msun_run}/bin/rsyncd.conf" /etc/
        chmod +x /etc/init.d/rsyncd
        cat > /etc/rsyncd.secrets <<EOF
#!/bin/sh
msun_rsync:msun_rsync
EOF
...
function rsync_client {
    ...
    mkdir -p /msun/rsync/nfs_img
    chown -R nobody.nobody /msun/rsync/
    if [ "$ans" == 'y' ] || [ "$ans" == 'yes' ]; then
        cat > /etc/cron.hourly/rsync <<EOF
#!/bin/sh
export RSYNC_PASSWORD='msun_rsync'
rsync -av msun_rsync@192.168.1.101::nfs_img /msun/rsync/nfs_img
EOF
    fi
    if [ "$ans" == 'n' ] || [ "$ans" == 'no' ]; then
...
function app_rsyslog_conf {
    ...
    if [ "$ans" == 'y' ] || [ "$ans" == 'yes' ]; then
        mkdir -p /msun/warrior/data/logger/
        chown -R warrior.admin /msun/warrior/data/
        cat >> ${rsyslog_conf} <<'EOF'

# our app log, notice that log below RileOwner will set to user warrior !
$template DailyAppLogs,"/msun/warrior/data/logger/app-%$YEAR%%$MONTH%%$DAY%.log"
$FileOwner warrior
local0.*                                                -?DailyAppLogs
EOF
